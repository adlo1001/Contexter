...
@SuppressLint("NewApi")
public class DrawGraphTwo extends ViewGroup {
	...
	public DrawGraphTwo(Context context, AttributeSet attrs_) {
	...
	public DrawGraphTwo(Context context, AttributeSet attrs, String Symbol) {
		super(context, attrs);
	...
	public void onDraw(Canvas canvas) {
		...
		if (temp_arr.size() > 1) {
			...
			for (Float temp_y : temp_arr) {
				...
				// canvas.drawLine(x_one, temp_y_previous + 0.5f * origin_y,
				// / x_two, temp_y + 0.5f * origin_y, mPaintfour);
		...
		canvas.drawText(y_two + " lx", origin_x + x_cl / 2, origin_y - y_cl / 2
				+ 20, mPainttwo);
		canvas.drawText("320.0 lx", origin_x + x_cl / 2, origin_y + y_cl / 2
				+ 20, mPainttwo);
		...
		canvas.drawText(this.sensor_name, origin_x - sensor_name.length() / 2,
	...
	public void setMonth(String sensor_) {
		this.sensor_name = sensor_;
		invalidate();
		requestLayout();
	}
	...
	public void setLimu(float parseFloat) {
		// TODO Auto-generated method stub
		this.curr_limunosity = parseFloat;

	}

	public Paint getmPainttwo() {
		return mPainttwo;
	}

	public void setmPainttwo(Paint mPainttwo) {
		this.mPainttwo = mPainttwo;
	}
