...
import se.sensiblethings.disseminationlayer.communication.rudp.socket.listener.RUDPLinkFailListener;
...
public class RUDPSender {
	...
	public void sendDatagram(RUDPDatagram datagram) throws InterruptedException,RUDPDestinationNotReachableException {
		...
		synchronized(this) {
			if(isShutdown) throw new RUDPDestinationNotReachableException();
		}
		...
		for (RUDPDatagramPacket packet : packetList) {
			...
			synchronized (this) {
				...
				if(isShutdown) {
					...
					throw new RUDPDestinationNotReachableException();
				}
